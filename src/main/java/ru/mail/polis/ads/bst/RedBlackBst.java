...
public class RedBlackBst<Key extends Comparable<Key>, Value>
    ...
    private Node root;
    private Value valueDelete = null;
...
//        return root == null ? null : get(root,key).value;
    ...
    private Node remove(Node node, Key key) {
        ...
        if (isKeyMoreNodeKey < 0) {
            ...
            } else if(isKeyMoreNodeKey == 0 && node.right == null){
                valueDelete = node.value;
                return null;
            } else if (node.right != null && !isRed(node.right) && !isRed(node.right.left)){
                    node = moveRedRight(node);
            }


            isKeyMoreNodeKey = (byte) key.compareTo(node.key);
            if(isKeyMoreNodeKey == 0){
                Node min = min(node.right);
                Node withOutMin = removeMin(node.right);

                valueDelete = node.value;

                node.key = min.key;
                node.value = min.value;
                node.right = withOutMin;
            } else {
                node.right = remove(node.right,key);
            }
